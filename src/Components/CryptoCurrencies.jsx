...
function CryptoCurrencies({simplified}) {
  ...
  useEffect(() => {    
    const filteredData = cryptoList?.data?.coins.filter((coin) => coin.name.toLowerCase().includes(searchCurrency.toLocaleLowerCase()));
  ...
  return (
    ...
    {/* The simplified will allow the search bar to hide from the homepage and only show on
      ...
      <div className="search-crypto">
        <Input placeholder="Search a Currency" onChange={(e) => setSearchCurrency(e.target.value)} />
        ...
        {cryptos?.map((currency) => (
          <Col xs={24} sm={12} lg={6} className="crypto-card" key={currency.id}>
            ...
            <Card
              ...
              hoverable
            >
              <p>Price: {millify(currency.price) < 1 ? millify(currency.price, {precision: 6}): millify(currency.price)}</p>
